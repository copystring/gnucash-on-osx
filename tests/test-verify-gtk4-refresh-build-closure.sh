...
glib_base_line="$(grep -Fn 'buildone --force --autogen --no-network glib-no-introspection' "$workflow" | cut -d: -f1)"
gi_line="$(grep -Fn 'buildone --force --no-network gobject-introspection' "$workflow" | cut -d: -f1)"
glib_line="$(grep -Fn 'buildone --force --autogen --no-network glib' "$workflow" | tail -n 1 | cut -d: -f1)"
...
test "$native_verify_line" -lt "$glib_base_line"
test "$glib_base_line" -lt "$gi_line"
